...
public class SseEmitterService {
        ...
        Disposable disposable = responseFlux
                .doOnNext(chunk -> {
                    try {
                        ...
                        log.warn("发�?SSE 事件失败: {}", e.getMessage());
                        throw new RuntimeException(e);
                    }
                })
                .doOnComplete(() -> {
                    try {
                        log.info("流式响应完成，总长�? {}", fullResponse.get().length());
    ...
    public SseEmitter createStreamEmitterWithAudio(Long conversationId,
        ...
        Disposable disposable = responseFlux
                ...
                .doOnComplete(() -> {
                    try {
                        log.info("音频流式响应完成，总长�? {}", fullResponse.get().length());
    ...
    public SseEmitter createStreamEmitterWithImage(Long conversationId,
        ...
        Disposable disposable = responseFlux
                ...
                .doOnComplete(() -> {
                    try {
                        log.info("图片流式响应完成，总长�? {}", fullResponse.get().length());
            ...
            while (toolCallCount < MAX_TOOL_CALLS) {
                ...
                if (toolCalls != null && !toolCalls.isEmpty()) {
                    ...
                    try {
                        ...
                        log.info("Agent 模式处理完成，响应长�? {}", textContent.length());
                    } catch (Exception e) {
                        log.error("发送文本响应失�?, e);
                        try {
                            StreamEvent errorEvent = StreamEvent.error(e.getMessage());
