    ...
    <div className="vocabulary-selector-wrapper">
      ...
      <select
        ...
        onChange={(e) => {
          ...
          preferences.setVocabularyCategory(newCategory)
          const firstDiff = VOCABULARY_DIFFICULTIES.find(d => d.category === newCategory)
          if (firstDiff) {
            preferences.setVocabularyDifficulty(firstDiff.difficulty)
          }
        }}
    ...
    if (learningMode === 'vocabulary') {
      if (intent === 'make_sentence' && currentVocabularyCard && response?.content) {
        try {
          ...
          const updatedCard = await vocabularyApi.updateAIFeedback(currentVocabularyCard.id, response.content)
          if (updatedCard) {
            useVocabularyStore.setState({ currentVocabularyCard: updatedCard })
          }
        } catch (e) {
          console.error('保存AI反馈失败:', e)
          try {
            const { vocabularyApi } = await import('../../../api')
            const card = await vocabularyApi.getCurrentCard(currentConversation.id)
            if (card) useVocabularyStore.setState({ currentVocabularyCard: card })
          } catch (err) {
            console.error('重新加载词汇卡失败:', err)
          }
        }
      } else {
        try {
      ...
      return (
        <div className="vocabulary-view">
          <VocabularyFlashcard
            ...
            onNextWord={() => handleNextWord(currentConversation!.id, currentVocabularyCard.position, vocabularyDifficulty)}
            onRegenerate={() => handleRegenerateWord(currentConversation!.id, vocabularyDifficulty)}
            onSaveMeaning={(meaning: string) => saveUserMeaning(currentVocabularyCard.id, meaning)}
            onSaveSentence={(sentence: string) => saveUserSentence(currentVocabularyCard.id, sentence)}
    ...
    return (
      <div className="vocabulary-view">
        <VocabularyFlashcard
          ...
          onNextWord={currentConversation ? () => handleNextWord(currentConversation.id, 0, vocabularyDifficulty) : undefined}
          onRegenerate={currentConversation ? () => handleRegenerateWord(currentConversation.id, vocabularyDifficulty) : undefined}
          isStandalone={true}
          isLoading={vocabularyCardLoading || loading}
        />
      </div>
